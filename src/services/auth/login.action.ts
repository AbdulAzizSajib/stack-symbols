...
import { ILoginResponse, type UserRole } from "@/types/auth.types";
...
export const loginAction = async (payload : ILoginPayload, redirectPath ?: string ) : Promise<ILoginResponse | ApiErrorResponse> =>{
    ...
    try {
        ...
        if (!loginData?.accessToken || !loginData?.refreshToken || !loginData?.token || !loginData?.user) {
            return {
                success: false,
                message: "Login failed: invalid response payload from server",
            };
        }

        const { accessToken, refreshToken, token, user } = loginData;
